...
// src/api/APISettingsStore.ts
var DEFAULT_VOICE_ID = "s3://peregrine-voices/larry_ads3_parrot_saad/manifest.json";
var DEFAULT_VOICE_ENGINE = "PlayHT2.0";
var APISettingsStore = class _APISettingsStore {
  constructor(settings) {
    this.settings = { ...settings };
    _APISettingsStore._instance = this;
  }
  static getSettings() {
    if (!_APISettingsStore._instance) {
      throw new Error(
    ...
    return _APISettingsStore._instance.settings;
  }
...
// src/api/generateV1Speech.ts
var WAIT_BETWEEN_STATUS_CHECKS_MS = 150;
var MAX_STATUS_CHECKS_RETRIES = 10;
async function generateV1Speech(content, voice, options) {
  ...
  const generationJobData = await axios_default.request(convertOptions).then(({ data }) => data).catch((error) => {
    throw {
  ...
  return await async function retryUntilGenerated() {
    do {
      const generationStatus = await axios_default.request(statusOptions).then(({ data }) => {
        return data;
      ...
      if (transcriped || converted) {
        const url2 = Array.isArray(audioUrl) ? audioUrl[0] : audioUrl;
        return {
          message,
          transcriptionId,
          audioUrl: url2
...
// src/api/generateV1Stream.ts
async function generateV1Stream(content, voice, options) {
...
// src/api/generateV2Speech.ts
import https2 from "https";
var CONNECTION_TIMEOUT = 30 * 1e3;
var INNACTIVITY_TIMEOUT = 20 * 1e3;
function generateV2Speech(text, voice, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https2.request(requestOptions, (res) => {
      res.on("data", (rawData) => {
        try {
      ...
      res.on("error", (error) => {
        if (error.message === "aborted") {
          reject({
            message: `The server response exceeded the maximum inactivity timeout of ${INNACTIVITY_TIMEOUT / 1e3}s`,
            status: 500
          });
        } else {
          reject({
    ...
    request.on("error", (error) => {
      if (error.message === "socket hang up") {
        reject({
          message: `Exceeded the maximum timeout of ${CONNECTION_TIMEOUT / 1e3}s while attempting to establish connection.`,
          status: 500
        });
      } else {
        reject({
...
// src/api/generateV2Stream.ts
async function generateV2Stream(text, voice, options) {
  const { apiKey, userId } = APISettingsStore.getSettings();
  ...
  const response = await axios_default(streamOptions).catch((error) => {
    throw {
      message: error.response?.data?.error_message || error.message,
      code: error.code,
      statusCode: error.response?.statusCode,
...
// src/api/textStreamToSentences.ts
var PUNCTUATION_REGEX = /[.!?:…]/;
async function* textStreamToSentences(inputStream) {
  let textInput = "";
  for await (const part of inputStream) {
    textInput += `${part}`;
    const punctuationMatch = textInput.match(PUNCTUATION_REGEX);
    if (punctuationMatch && punctuationMatch.index !== void 0) {
...
// src/api/apiCommon.ts
async function commonGenerateSpeech(input, optionsInput) {
...
async function commonGenerateStream(input, optionsInput) {
...
async function internalGenerateStreamFromString(input, optionsInput) {
  const options = addDefaultOptions(optionsInput);
  if (options.voiceEngine === "Standard") {
    const v1Options = toV1Options(options);
...
async function internalGenerateStreamFromInputStream(inputStream, options) {
...
function toV2Options(options, isPlay20Streaming = false) {
  ...
  if (options.voiceEngine !== "PlayHT1.0" && options.voiceEngine !== "PlayHT2.0" && options.voiceEngine !== "PlayHT2.0-turbo") {
    throw {
      message: "Invalid engine. Expected 'PlayHT2.0', 'PlayHT2.0-turbo' or 'PlayHT1.0'",
      code: "INVALID_ENGINE"
    };
  }
  const quality = options.quality ?? (options.voiceEngine === "PlayHT2.0" ? "high" : options.voiceEngine === "PlayHT2.0-turbo" ? "draft" : void 0);
  const v2Options = {
    quality,
...
var CONCURRENCY = 5;
async function audioStreamFromSentences(sentencesStream, writableStream, options) {
  const concurrencyQueue = new dist_default({ concurrency: CONCURRENCY });
...
// src/api/availableClonedVoices.ts
function convertResponseToVoiceInfo(voice) {
  return {
    voiceEngine: voice.voice_engine || "PlayHT1.0",
    isCloned: true,
    id: voice.id,
    name: voice.name,
    gender: voice.gender
...
async function availableClonedVoices() {
  const { apiKey, userId } = APISettingsStore.getSettings();
  const options = {
    method: "GET",
  ...
  return await axios_default.request(options).then(({ data }) => data.map(convertResponseToVoiceInfo)).catch(function(error) {
    throw {
      message: error.response?.data?.error_message || error.message,
      code: error.code,
      statusCode: error.response?.statusCode,
...
// src/api/availableV1Voices.ts
var CACHE_EXPIRE_TIME = 1e3 * 60 * 60 * 12;
var _v1VoicesCache;
var _cacheUpdatedTime;
async function availableV1Voices() {
  const { apiKey, userId } = APISettingsStore.getSettings();
  const options = {
    method: "GET",
  ...
  _v1VoicesCache = await axios_default.request(options).then(
    ({
      data
      ...
      id: v1Voice.value,
      name: v1Voice.name,
      sampleUrl: v1Voice.sample ? v1Voice.sample : void 0,
      language: v1Voice.language,
      languageCode: v1Voice.languageCode,
      // Use ternary operator to get correct type.
      gender: v1Voice.gender.toLocaleLowerCase() === "male" ? "male" : v1Voice.gender.toLocaleLowerCase() === "female" ? "female" : void 0,
...
// src/api/availableV2Voices.ts
var CACHE_EXPIRE_TIME2 = 1e3 * 60 * 60 * 12;
var _v2VoicesCache;
var _cacheUpdatedTime2;
async function availableV2Voices() {
  const { apiKey, userId } = APISettingsStore.getSettings();
  const options = {
    method: "GET",
  ...
  if (_v2VoicesCache && _cacheUpdatedTime2 && Date.now() - _cacheUpdatedTime2 < CACHE_EXPIRE_TIME2) {
    return _v2VoicesCache;
  }
  _v2VoicesCache = await axios_default.request(options).then(
    ({ data }) => data.map((v2Voice) => ({
      voiceEngine: v2Voice.voice_engine || "PlayHT1.0",
      id: v2Voice.id,
      name: v2Voice.name,
      sampleUrl: v2Voice.sample ? v2Voice.sample : void 0,
      language: v2Voice.language,
      languageCode: v2Voice.language_code,
      gender: v2Voice.gender,
      ageGroup: toAgeGroup(v2Voice.age),
      styles: v2Voice.style ? [v2Voice.style] : void 0,
      isCloned: false
    }))
  ).catch(function(error) {
    throw {
...
// src/api/commonGetAllVoices.ts
async function commonGetAllVoices(filters) {
  const needV1Voices = filters?.isCloned !== true && (!filters || !filters.voiceEngine || filters.voiceEngine.includes("Standard"));
  const needV2Voices = filters?.isCloned !== true && (!filters || !filters.voiceEngine || filters.voiceEngine.includes("PlayHT1.0") || filters.voiceEngine.includes("PlayHT2.0"));
  const needClonedVoices = !filters || filters.isCloned === void 0 || filters.isCloned;
  ...
  return [...clonedVoices, ...v2Voices, ...v1Voices].filter((voice) => {
    ...
    if (filters?.isCloned !== void 0 && voice.isCloned !== filters.isCloned) {
      return false;
    }
    return true;
  });
...
var StringType = class {
  ...
  get(uint8Array, offset) {
    return Buffer3.from(uint8Array).toString(this.encoding, offset, offset + this.len);
  }
};
...
var FileTypeParser = class {
  ...
  async parse(tokenizer) {
    this.buffer = Buffer4.alloc(minimumBytes);
    ...
    if (this.checkString("%PDF")) {
      try {
        ...
        const buffer = Buffer4.alloc(Math.min(maxBufferSize, tokenizer.fileInfo.size));
        await tokenizer.readBuffer(buffer, { mayBeLess: true });
        if (buffer.includes(Buffer4.from("AIPrivateData"))) {
          return {
            ext: "ai",
            mime: "application/postscript"
          };
...
// src/api/instantCloneInternal.ts
var CLONE_API_URL = "https://play.ht/api/v2/cloned-voices/instant";
var DELETE_API_URL = "https://play.ht/api/v2/cloned-voices";
async function commonInstantClone(voiceName, input, voiceGender, mimeType) {
  if (typeof input === "string") {
...
async function internalInstantClone(voiceName, input, voiceGender, mimeType) {
  const { apiKey, userId } = APISettingsStore.getSettings();
  ...
  return await axios_default.post(CLONE_API_URL, formData, {
    headers: {
      accept: "application/json",
      AUTHORIZATION: apiKey,
      "X-USER-ID": userId,
  ...
  }).then(({ data }) => convertResponseToVoiceInfo(data)).catch(function(error) {
    throw {
      message: error.response?.data?.error_message || error.message,
      code: error.code,
      statusCode: error.response?.statusCode,
...
async function internalDeleteClone(voiceId) {
  const { apiKey, userId } = APISettingsStore.getSettings();
  return await axios_default.delete(DELETE_API_URL, {
    headers: {
      accept: "application/json",
      AUTHORIZATION: apiKey,
      "X-USER-ID": userId,
...
async function stream4(input, options) {
  return await commonGenerateStream(input, options);
}
async function clone(voiceName, input, voiceGender, mimeType) {
